...
// GetPublicKeyBytes returns the 32-byte enrolled public key array, or false if unenrolled or invalid length.
func (v *Ed25519Verifier) GetPublicKeyBytes() ([32]byte, bool) {
	var out [32]byte
	v.keyMu.RLock()
	defer v.keyMu.RUnlock()
	if !v.isEnrolled || len(v.publicKey) != ed25519.PublicKeySize {
		return out, false
	}

...
func InspectModelMetadata(modelIdentifier string, hfTokens ...string) ModelArchitectureMeta {
	...
	cleanModelID := modelIdentifier
	if idx := strings.Index(cleanModelID, ":"); idx != -1 {
		cleanModelID = cleanModelID[:idx]
	}
	...
	candidates := []string{
		...
		filepath.Join(cleanModelID, "config.json"),
		filepath.Join("/opt/models", modelIdentifier, "config.json"),
		filepath.Join("/opt/models", cleanModelID, "config.json"),
		filepath.Join("/mnt/ramdisk/models", modelIdentifier, "config.json"),
		filepath.Join("/mnt/ramdisk/models", cleanModelID, "config.json"),
	}

	// Check huggingface cache paths
	hfCacheRepo := "models--" + strings.ReplaceAll(cleanModelID, "/", "--")
	...
	if !foundLocal && strings.Contains(cleanModelID, "/") && !filepath.IsAbs(cleanModelID) && !strings.HasPrefix(cleanModelID, ".") {
		token := ""
		if len(hfTokens) > 0 {
			token = hfTokens[0]
		}
		if remoteCfg := fetchRemoteHFConfig(cleanModelID, token); remoteCfg != nil {
	...
	if meta.ModelNativeMax <= 4096 {
		switch {
		case strings.Contains(lower, "-1m"), strings.Contains(lower, "1m-"), strings.Contains(lower, ":1m"):
			meta.ModelNativeMax = 1048576
		case strings.Contains(lower, "-512k"), strings.Contains(lower, "512k-"), strings.Contains(lower, ":512k"):
			meta.ModelNativeMax = 524288
		case strings.Contains(lower, "-256k"), strings.Contains(lower, "256k-"), strings.Contains(lower, ":256k"):
			meta.ModelNativeMax = 262144
		case strings.Contains(lower, "-128k"), strings.Contains(lower, "128k-"), strings.Contains(lower, ":128k"):
			meta.ModelNativeMax = 131072
		case strings.Contains(lower, "-64k"), strings.Contains(lower, "64k-"), strings.Contains(lower, ":64k"):
			meta.ModelNativeMax = 65536
		case strings.Contains(lower, "qwen2.5"), strings.Contains(lower, "qwen2"), strings.Contains(lower, "qwen3"):
			meta.ModelNativeMax = 32768 // 32k max_position_embeddings in config.json
...
func alignContextLength(hardwareMax, modelNativeMax int) int {
	...
	// Standard transformer context intervals up to 1M
	standardBuckets := []int{2048, 4096, 8192, 16384, 32768, 65536, 131072, 262144, 524288, 1048576}
...
// FormatContextSize returns a human-readable token representation (e.g. 32K, 1M).
func FormatContextSize(tokens int) string {
	if tokens >= 1048576 && tokens%1048576 == 0 {
		return strconv.Itoa(tokens/1048576) + "M"
	}
	if tokens >= 1024 {
...
func fetchRemoteHFConfig(repoID, token string) *hfConfigJSON {
	cleanRepo := repoID
	if idx := strings.Index(cleanRepo, ":"); idx != -1 {
		cleanRepo = cleanRepo[:idx]
	}
	url := fmt.Sprintf("https://huggingface.co/%s/raw/main/config.json", cleanRepo)
	client := &http.Client{

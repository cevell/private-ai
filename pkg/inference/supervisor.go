...
type ModelSpec struct {
	...
	Quantization    string  `json:"quantization,omitempty"`      // Explicit quantization ("bitsandbytes", "gguf", "fp8", "awq", "gptq")
	BitsAndBytes    *bool   `json:"bitsandbytes,omitempty"`      // Enable BitsAndBytes quantization plugin (--quantization bitsandbytes)
	GGUF            *bool   `json:"gguf,omitempty"`              // Enable GGUF format plugin (--quantization gguf)
	Tokenizer       string  `json:"tokenizer,omitempty"`         // Explicit tokenizer override or base Hugging Face repo for GGUF models
	...
	ToolCallParser  string  `json:"tool_call_parser,omitempty"`  // Explicit tool call parser override ("hermes", "llama3_json", "mistral", etc.)
	EnableThinking  *bool   `json:"enable_thinking,omitempty"`   // Explicit thinking / reasoning enable flag
	ReasoningParser string  `json:"reasoning_parser,omitempty"`  // Explicit reasoning parser override ("deepseek_r1", etc.)
}
...
// chownRecursive recursively sets ownership and directory permissions across a filesystem tree.
...
// and ensures temporary JIT directories exist with correct unprivileged permissions.
func cleanupStaleCacheArtifacts() {
	...
	for _, sub := range []string{
		...
		"/mnt/ramdisk/tmp/flashinfer",
		"/mnt/ramdisk/home/.cache/flashinfer",
	} {
		_ = os.RemoveAll(sub)
	}
	for _, dir := range []string{
		...
		"/mnt/ramdisk/tmp/torch_ext",
		"/mnt/ramdisk/tmp/flashinfer",
		"/mnt/ramdisk/home/.cache",
		"/mnt/ramdisk/home/.cache/flashinfer",
		"/mnt/ramdisk/cache",
		"/mnt/ramdisk/cache/vllm",
	} {
	...
	if os.Getuid() == 0 {
		chownRecursive("/mnt/ramdisk/home", 1000, 1000)
		chownRecursive("/mnt/ramdisk/tmp", 1000, 1000)
		chownRecursive("/mnt/ramdisk/cache", 1000, 1000)
	}
}
...
func (s *Supervisor) LoadModel(ctx context.Context, spec ModelSpec) error {
	...
	if spec.Tokenizer != "" && (strings.HasPrefix(spec.Tokenizer, "-") || strings.ContainsAny(spec.Tokenizer, "\x00\r\n\t;`$|&><")) {
		return fmt.Errorf("invalid tokenizer parameter: contains forbidden characters or flag prefix")
	}
	...
	if (spec.BitsAndBytes != nil && *spec.BitsAndBytes) ||
		spec.Quantization == "bitsandbytes" ||
		spec.Quantization == "bnb" ||
		(spec.GGUF != nil && *spec.GGUF) ||
		spec.Quantization == "gguf" ||
		strings.HasSuffix(strings.ToLower(spec.Model), ".gguf") ||
		strings.HasSuffix(strings.ToLower(spec.HFFile), ".gguf") {
...
// ResolveToolCallParser determines the appropriate vLLM tool-call parser for a given model.
// Supported parsers in vLLM: hermes, llama3_json, mistral, granite, granite-20b-fc, internlm, jamba, pythonic.
func ResolveToolCallParser(modelName string, override string) (string, bool) {
	if override != "" {
		return override, true
	}
	m := strings.ToLower(modelName)
	switch {
	// Llama 3.2 models frequently use pythonic function syntax
	case strings.Contains(m, "llama-3.2") || strings.Contains(m, "llama3.2"):
		return "pythonic", true
	// Llama 3, 3.1, 3.3 models use JSON schema headers
	case strings.Contains(m, "llama"):
		return "llama3_json", true
	// Mistral, Mixtral, Codestral, Pixtral use [TOOL_CALLS]
	case strings.Contains(m, "mistral") || strings.Contains(m, "mixtral") || strings.Contains(m, "codestral") || strings.Contains(m, "pixtral"):
		return "mistral", true
	// Granite family
	case strings.Contains(m, "granite-20b"):
		return "granite-20b-fc", true
	...
	case strings.Contains(m, "deepseek") && strings.Contains(m, "llama"):
		return "llama3_json", true
	// Qwen (2.5/2/1.5), DeepSeek (V2/V3/R1-Distill-Qwen/Coder), GLM-4/ChatGLM, Kimi / Moonlight, Yi, Hermes
	// All use ChatML / XML tool call tags (<tool_call>) parsed by "hermes"
	case strings.Contains(m, "qwen") || strings.Contains(m, "deepseek") || strings.Contains(m, "glm") ||
		strings.Contains(m, "kimi") || strings.Contains(m, "moonlight") || strings.Contains(m, "hermes") ||
		strings.Contains(m, "yi") || strings.Contains(m, "chatglm"):
		return "hermes", true
	default:
		// Default to hermes parser for general ChatML tool definitions
		return "hermes", true
	}
}

// ResolveReasoningParser determines whether vLLM reasoning_content should be enabled.
func ResolveReasoningParser(modelName string, enableOverride *bool, parserOverride string) (string, bool) {
	if enableOverride != nil && !*enableOverride {
		return "", false
	}
	if enableOverride != nil && *enableOverride {
		if parserOverride != "" {
			return parserOverride, true
		}
		return "deepseek_r1", true
	}
	m := strings.ToLower(modelName)
	if strings.Contains(m, "r1") || strings.Contains(m, "reason") || strings.Contains(m, "qwq") ||
		strings.Contains(m, "thinking") || strings.Contains(m, "marco-o1") {
		if parserOverride != "" {
			return parserOverride, true
		}
		return "deepseek_r1", true
...
// BuildVLLMArgs constructs the command-line arguments for launching vLLM with architecture-aware tuning.
func BuildVLLMArgs(spec ModelSpec, topo hardware.TopologyReport, tensorParallel int, port int, prefixArgs ...string) []string {
	modelArg := spec.Model
	if spec.HFFile != "" {
		if !strings.Contains(modelArg, spec.HFFile) {
			modelArg = strings.TrimSuffix(modelArg, "/") + "/" + spec.HFFile
		}
	} else if strings.Contains(modelArg, ":") && strings.HasSuffix(strings.ToLower(modelArg), ".gguf") {
		// Convert repo:file.gguf to repo/file.gguf so vllm-gguf-plugin downloads the explicit file
		idx := strings.LastIndex(modelArg, ":")
		modelArg = modelArg[:idx] + "/" + modelArg[idx+1:]
	}
	...
	// Quantization plugin injection (BitsAndBytes, GGUF, or explicit quantization)
	isBNB := (spec.BitsAndBytes != nil && *spec.BitsAndBytes) ||
		spec.Quantization == "bitsandbytes" ||
		spec.Quantization == "bnb"

	if isBNB {
		system.Log("[Plugins] Activating BitsAndBytes quantization plugin (--quantization bitsandbytes --load-format bitsandbytes)")
		args = append(args, "--quantization", "bitsandbytes", "--load-format", "bitsandbytes")
	} else if (spec.GGUF != nil && *spec.GGUF) ||
		spec.Quantization == "gguf" ||
		strings.HasSuffix(strings.ToLower(spec.Model), ".gguf") ||
		strings.HasSuffix(strings.ToLower(spec.HFFile), ".gguf") {
		system.Log("[Plugins] Activating GGUF format plugin (--quantization gguf)")
		args = append(args, "--quantization", "gguf")
	} else if spec.Quantization != "" {
		system.Log("[Quantization] Applying explicit quantization flag: %s", spec.Quantization)
		args = append(args, "--quantization", spec.Quantization)
	}

	// Stability & Numerical Integrity Tuning:
	// 1. Disable prefix caching to prevent cross-request KV cache block boundary corruption and guarantee confidential session isolation.
	// 2. Retain vLLM v1 native chunked prefill (required for v1 attention scheduling and memory allocation).
	// 3. Force FlashAttention version 2 via --attention-config '{"flash_attn_version": 2}' on Hopper, Ada, and Ampere:
	//    - On Hopper SM90: FA2 prevents FlashAttention-3 numerical precision drift and NaN logit generation.
	//    - On Ada SM89 (L4) and Ampere SM80 (A100): FA2 is the native optimal attention kernel.
	//    - On Blackwell (SM100+, B100/B200/RTX PRO 6000): do NOT force FA2 (FA2 has no SM100 binaries; Blackwell uses native FA3/FA4/TMA kernels).
	//    - On Turing (SM75, T4): do NOT pass --attention-config (Turing uses native/XFormers).
	// 4. Architecture-specific execution mode:
	//    - On Hopper (SM90): enforce eager mode to prevent PyTorch Inductor / CUDAGraph NaN collapse on vLLM 0.29.0 / PyTorch 2.13.0.
	//    - On Turing (SM75): enforce eager mode (no modern CUDAGraph support).
	//    - On BitsAndBytes (isBNB): enforce eager mode (4-bit quantization does not support graph capture).
	//    - On Blackwell (SM100+) and Ada (L4): allow CUDAGraphs and compiled kernels to accelerate token generation by 2-3x.
	args = append(args, "--no-enable-prefix-caching")

	isHopper := topo.IsHopperGPU || (topo.ComputeCapability >= 9.0 && topo.ComputeCapability < 10.0) || strings.Contains(strings.ToUpper(topo.GPUModel), "H100") || strings.Contains(strings.ToUpper(topo.GPUModel), "H200")
	isBlackwell := topo.IsBlackwellGPU || topo.ComputeCapability >= 10.0 || strings.Contains(strings.ToUpper(topo.GPUModel), "B100") || strings.Contains(strings.ToUpper(topo.GPUModel), "B200") || strings.Contains(strings.ToUpper(topo.GPUModel), "RTX PRO 6000") || strings.Contains(strings.ToUpper(topo.GPUModel), "BLACKWELL") || strings.Contains(strings.ToUpper(topo.GPUModel), "B40")
	isAda := topo.IsAdaGPU || (topo.ComputeCapability >= 8.9 && topo.ComputeCapability < 9.0) || strings.Contains(strings.ToUpper(topo.GPUModel), "L4") || strings.Contains(strings.ToUpper(topo.GPUModel), "ADA")
	isAmpere := topo.IsAmpereGPU || (topo.ComputeCapability >= 8.0 && topo.ComputeCapability < 8.9) || strings.Contains(strings.ToUpper(topo.GPUModel), "A100") || strings.Contains(strings.ToUpper(topo.GPUModel), "AMPERE")

	// Attention Backend Dispatch
	if isHopper || isAda || isAmpere || (!topo.IsTuringGPU && !isBlackwell && topo.ComputeCapability < 10.0 && topo.ComputeCapability >= 8.0) {
		args = append(args, "--attention-config", `{"flash_attn_version": 2}`)
	}

	// Execution Mode Dispatch (CUDAGraphs vs Eager)
	if isHopper || topo.IsTuringGPU || isBNB {
		args = append(args, "--enforce-eager")
	}

	// Architecture-aware dtype tuning:
	// - On Turing (Tesla T4, CC 7.5): force dtype half (float16) because Turing does not support native bfloat16.
	// - On Ampere/Ada/Hopper/Blackwell (CC 8.0+): auto dtype (bfloat16).
	if topo.IsTuringGPU {
		args = append(args, "--dtype", "half")
	} else {
		args = append(args, "--dtype", "auto")
	}

	// Tool call parser auto-detection and enablement
	if toolParser, enabled := ResolveToolCallParser(spec.Model, spec.ToolCallParser); enabled {
		system.Log("[Capabilities] Enabling auto tool choice with parser %q for model %s", toolParser, spec.Model)
		args = append(args, "--enable-auto-tool-choice", "--tool-call-parser", toolParser)
	}

	// Reasoning / thinking parser auto-detection and enablement (DeepSeek-R1, QwQ, etc.)
	if reasoningParser, enabled := ResolveReasoningParser(spec.Model, spec.EnableThinking, spec.ReasoningParser); enabled {
		system.Log("[Capabilities] Enabling reasoning content parser %q for model %s", reasoningParser, spec.Model)
		args = append(args, "--reasoning-parser", reasoningParser)
	}

	// Tokenizer override or auto-derivation for standalone GGUF repositories
	if spec.Tokenizer != "" {
		system.Log("[Tokenizer] Applying explicit tokenizer override: %s", spec.Tokenizer)
		args = append(args, "--tokenizer", spec.Tokenizer)
	} else if (spec.GGUF != nil && *spec.GGUF) || spec.Quantization == "gguf" {
		baseRepo := spec.Model
		if idx := strings.Index(baseRepo, ":"); idx != -1 {
			baseRepo = baseRepo[:idx]
		}
		if strings.HasSuffix(strings.ToLower(baseRepo), "-gguf") {
			derivedTokenizer := baseRepo[:len(baseRepo)-5]
			system.Log("[Plugins] Auto-derived tokenizer %q for standalone GGUF model %s", derivedTokenizer, spec.Model)
			args = append(args, "--tokenizer", derivedTokenizer)
		}
	}
...
func (s *Supervisor) loadGPUModel(ctx context.Context, spec ModelSpec, meta ModelArchitectureMeta) error {
	...
	args := BuildVLLMArgs(spec, topo, tensorParallel, s.port, prefixArgs...)
	...
	cudaHome := findCUDAHome()
	setupCUDALibrarySymlinks(cudaHome)

	cmd := exec.Command(binPath, args...)
	configureSandboxProcAttr(cmd)
	cmd.Env = []string{
		fmt.Sprintf("PATH=%s/bin:/mnt/ramdisk/vllm/bin:/usr/local/cuda/bin:/usr/bin:/usr/sbin:/bin:/sbin", cudaHome),
		"VIRTUAL_ENV=/mnt/ramdisk/vllm",
		"PYTHONPATH=/mnt/ramdisk/vllm/lib/python3.12/site-packages",
		"PYTHONEXECUTABLE=/mnt/ramdisk/vllm/bin/python3",
		"HOME=/mnt/ramdisk/home",
		"HF_HOME=/mnt/ramdisk/cache",
		"HF_XET_HIGH_PERFORMANCE=1",
		"TMPDIR=/mnt/ramdisk/tmp",
		"FLASHINFER_WORKSPACE_DIR=/mnt/ramdisk/tmp/flashinfer",
		"TORCH_HOME=/mnt/ramdisk/cache",
		"XDG_CACHE_HOME=/mnt/ramdisk/cache",
		"VLLM_CACHE_ROOT=/mnt/ramdisk/cache/vllm",
		"TRITON_CACHE_DIR=/mnt/ramdisk/tmp/triton",
		"TRITON_LIBCUDA_PATH=/usr/lib/x86_64-linux-gnu",
		fmt.Sprintf("CUDA_HOME=%s", cudaHome),
		fmt.Sprintf("CUDA_PATH=%s", cudaHome),
		"CC=/usr/bin/gcc",
		"CXX=/usr/bin/g++",
		...
		fmt.Sprintf("LD_LIBRARY_PATH=%s/lib:%s/lib64:/mnt/ramdisk/vllm/lib:/mnt/ramdisk/vllm/lib64:/usr/lib/x86_64-linux-gnu:/usr/lib64:/lib64:/lib:/usr/local/cuda/lib64:/usr/local/cuda/lib", cudaHome, cudaHome),
	}
	if topo.GPUCount > 1 {
...
func ensureVLLMEnvironment() error {
	...
	cleanupStaleCacheArtifacts()
	setupCUDALibrarySymlinks(findCUDAHome())
	...
	system.Log("Installing GPU-locked vLLM packages into /mnt/ramdisk/vllm...")
...
// patchVLLMSampler ensures vLLM's PyTorch fallback sampler (when FlashInfer is disabled)
// handles unconstrained top_k (k <= 0 or k == -1) safely without out-of-bounds CUDA indexing assertion.
func patchVLLMSampler() {
	matches, err := filepath.Glob("/mnt/ramdisk/vllm/lib/python*/site-packages/vllm/v1/sample/ops/topk_topp_sampler.py")
	if err != nil || len(matches) == 0 {
		return
	}
	re := regexp.MustCompile(`(?s)if k is not None:\s+# Apply top-k\.\s+top_k_mask = logits_sort\.size\(1\) - k\.to\(torch\.long\).*?logits_sort\.masked_fill_\(top_k_mask, -float\("inf"\)\)`)

	replacement := []byte(`if k is not None:
        # Apply top-k.
        valid_k = (k > 0) & (k < logits_sort.size(1))
        if valid_k.any():
            safe_k = torch.clamp(k, min=1, max=logits_sort.size(1))
            top_k_mask = logits_sort.size(1) - safe_k.to(torch.long)  # shape: B
            # Get all the top_k values.
            top_k_val = logits_sort.gather(1, top_k_mask.unsqueeze(dim=1))
            mask = (logits_sort < top_k_val) & valid_k.unsqueeze(dim=1)
            logits_sort.masked_fill_(mask, -float("inf"))`)

	for _, target := range matches {
		data, err := os.ReadFile(target)
		if err != nil {
			system.LogError("Failed to read sampler file %s: %v", target, err)
			continue
		}
		if bytes.Contains(data, []byte("valid_k = (k > 0)")) {
			system.Log("Sampler %s already patched", target)
			continue
		}
		if !re.Match(data) {
			system.LogError("Notice: sampler %s did not match regex pattern", target)
			continue
		}
		patched := re.ReplaceAll(data, replacement)
		if err := os.WriteFile(target, patched, 0644); err == nil {
			system.Log("Successfully applied PyTorch fallback sampler bounds-check patch to %s", target)
		} else {
			system.LogError("Failed to write patched sampler to %s: %v", target, err)
		}
	}
}

// findCUDAHome locates the active CUDA toolkit root (either pip cu13/cu12 wheel or system /usr/local/cuda).
func findCUDAHome() string {
	candidates := []string{
		"/mnt/ramdisk/vllm/lib/python3.12/site-packages/nvidia/cu13",
		"/mnt/ramdisk/vllm/lib/python3.12/site-packages/nvidia/cu12",
		"/usr/local/cuda",
	}
...
// setupCUDALibrarySymlinks ensures required dynamic library symlinks exist for FlashInfer JIT compilation
// (lib64, libcudart.so, and driver libcuda.so in the CUDA toolkit directory).
func setupCUDALibrarySymlinks(cudaHome string) {
	if cudaHome == "" {
		return
	}
	libDir := filepath.Join(cudaHome, "lib")
	lib64Dir := filepath.Join(cudaHome, "lib64")

	if fi, err := os.Stat(libDir); err == nil && fi.IsDir() {
		if _, err := os.Lstat(lib64Dir); err != nil {
			_ = os.Symlink("lib", lib64Dir)
			if os.Getuid() == 0 {
				_ = os.Lchown(lib64Dir, 1000, 1000)
			}
		}

		// libcudart.so -> libcudart.so.X
		cudartSo := filepath.Join(libDir, "libcudart.so")
		if _, err := os.Lstat(cudartSo); err != nil {
			matches, _ := filepath.Glob(filepath.Join(libDir, "libcudart.so.*"))
			if len(matches) > 0 {
				base := filepath.Base(matches[0])
				_ = os.Symlink(base, cudartSo)
				if os.Getuid() == 0 {
					_ = os.Lchown(cudartSo, 1000, 1000)
				}
				system.Log("Created CUDA runtime symlink: %s -> %s", cudartSo, base)
			}
		}

		// libcuda.so -> host driver libcuda.so.1
		cudaSo := filepath.Join(libDir, "libcuda.so")
		if _, err := os.Lstat(cudaSo); err != nil {
			...
			for _, dc := range driverCandidates {
				if _, err := os.Stat(dc); err == nil {
					_ = os.Symlink(dc, cudaSo)
					if os.Getuid() == 0 {
						_ = os.Lchown(cudaSo, 1000, 1000)
					}
					system.Log("Created CUDA driver symlink: %s -> %s", cudaSo, dc)
					break
				}
			}
		}
	...
	// Also ensure /mnt/ramdisk/vllm/lib64 symlink exists if lib exists
	if fi, err := os.Stat("/mnt/ramdisk/vllm/lib"); err == nil && fi.IsDir() {
		vllmLib64 := "/mnt/ramdisk/vllm/lib64"
		if _, err := os.Lstat(vllmLib64); err != nil {
			_ = os.Symlink("lib", vllmLib64)
			if os.Getuid() == 0 {
				_ = os.Lchown(vllmLib64, 1000, 1000)
			}
		}
	}
}
...
func findGPUInferenceBinary() (string, []string, string) {
	if _, err := os.Stat("/mnt/ramdisk/vllm/bin/python3"); err == nil {
		return "/mnt/ramdisk/vllm/bin/python3", []string{"-m", "vllm.entrypoints.cli.main", "serve"}, "vllm"
	}
	if _, err := os.Stat("/mnt/ramdisk/vllm/bin/vllm"); err == nil {

...
func TestInspectModelMetadataHeuristics(t *testing.T) {
	...
	// 4. Long context tag detection (1M, 512K, 256K)
	m1m := InspectModelMetadata("Qwen/Qwen2-7B-Instruct-1M")
	assert.Equal(t, 1048576, m1m.ModelNativeMax)
	m512k := InspectModelMetadata("custom-model-512k")
	assert.Equal(t, 524288, m512k.ModelNativeMax)
}
...
func TestAlignContextLength(t *testing.T) {
	// Tests power-of-two / standard bucket alignment up to 1M
	assert.Equal(t, 1048576, alignContextLength(1100000, 1048576))
	assert.Equal(t, 524288, alignContextLength(600000, 1048576))
	assert.Equal(t, 262144, alignContextLength(300000, 1048576))
...
func TestFormatContextSize(t *testing.T) {
	assert.Equal(t, "1M", FormatContextSize(1048576))
	assert.Equal(t, "512K", FormatContextSize(524288))

package attestation

import (
	"bytes"
	"crypto/x509"
	"encoding/pem"
	"os"
	"strings"

	pb "github.com/google/go-sev-guest/proto/sevsnp"
)

// Official AMD Root & Intermediate CA Chains embedded directly as Go constants.
const (
	AMDChainMilanPEM = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`

	AMDChainGenoaPEM = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`

	AMDChainTurinPEM = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`

	AMDVLEKChainMilanPEM = `-----BEGIN CERTIFICATE-----

0YPk
-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`

	AMDVLEKChainGenoaPEM = `-----BEGIN CERTIFICATE-----

2cko
-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`

	AMDVLEKChainTurinPEM = `-----BEGIN CERTIFICATE-----

SUxr
-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`
)

// FormatCertificateChain constructs a complete, valid PEM certificate chain for AMD SEV-SNP.
func FormatCertificateChain(chain *pb.CertificateChain) string {
	if chain == nil {
		if IsCloudHypervisor() {
			return ResolveAMDVLEKChain()
		}
		return ResolveAMDVCEKChain()
	}

	var buf bytes.Buffer
	hasLeaf := false
	var leafBytes []byte
	isVlek := false
	hasAsk := false
	hasArk := false

	// 1. Leaf Certificate (VLEK or VCEK)
	if len(chain.VlekCert) > 0 {
		leafBytes = chain.VlekCert
		pem.Encode(&buf, &pem.Block{Type: "CERTIFICATE", Bytes: chain.VlekCert})
		hasLeaf = true
		isVlek = true
	} else if len(chain.VcekCert) > 0 {
		leafBytes = chain.VcekCert
		pem.Encode(&buf, &pem.Block{Type: "CERTIFICATE", Bytes: chain.VcekCert})
		hasLeaf = true
	}

	// 2. Intermediate CA (ASK or ASVK)
	if len(chain.AskCert) > 0 {
		pem.Encode(&buf, &pem.Block{Type: "CERTIFICATE", Bytes: chain.AskCert})
		hasAsk = true
	}

	// 3. Root CA (ARK)
	if len(chain.ArkCert) > 0 {
		pem.Encode(&buf, &pem.Block{Type: "CERTIFICATE", Bytes: chain.ArkCert})
		hasArk = true
	}

	// Dynamic fallback if hypervisor omitted ASK or ARK:
	if !hasAsk || !hasArk {
		if hasLeaf && len(leafBytes) > 0 {
			if cert, err := x509.ParseCertificate(leafBytes); err == nil {
				issuerCN := strings.ToLower(cert.Issuer.CommonName)
				subjectCN := strings.ToLower(cert.Subject.CommonName)

				if strings.Contains(issuerCN, "vlek") || strings.Contains(subjectCN, "vlek") || isVlek {
					switch {
					case strings.Contains(issuerCN, "genoa") || strings.Contains(subjectCN, "genoa"):
						buf.WriteString(AMDVLEKChainGenoaPEM)
					case strings.Contains(issuerCN, "turin") || strings.Contains(subjectCN, "turin"):
						buf.WriteString(AMDVLEKChainTurinPEM)
					default:
						buf.WriteString(GetAMDVLEKChain(DetectCPUGeneration()))
					}
				} else {
					switch {
					case strings.Contains(issuerCN, "genoa") || strings.Contains(subjectCN, "genoa"):
						buf.WriteString(AMDChainGenoaPEM)
					case strings.Contains(issuerCN, "turin") || strings.Contains(subjectCN, "turin"):
						buf.WriteString(AMDChainTurinPEM)
					default:
						buf.WriteString(GetAMDVCEKChain(DetectCPUGeneration()))
					}
				}
			} else {
				if isVlek || IsCloudHypervisor() {
					buf.WriteString(ResolveAMDVLEKChain())
				} else {
					buf.WriteString(ResolveAMDVCEKChain())
				}
			}
		} else {
			if isVlek || IsCloudHypervisor() {
				buf.WriteString(ResolveAMDVLEKChain())
			} else {
				buf.WriteString(ResolveAMDVCEKChain())
			}
		}
	}

	if !hasLeaf && buf.Len() == 0 {
		if IsCloudHypervisor() {
			return ResolveAMDVLEKChain()
		}
		return ResolveAMDVCEKChain()
	}

	return buf.String()
}

// IsCloudHypervisor tests whether the virtual machine is running under AWS Nitro or major cloud hypervisors.
func IsCloudHypervisor() bool {
	paths := []string{
		"/sys/class/dmi/id/sys_vendor",
		"/sys/class/dmi/id/bios_vendor",
		"/sys/class/dmi/id/product_name",
		"/sys/devices/virtual/dmi/id/sys_vendor",
		"/sys/devices/virtual/dmi/id/product_name",
	}
	for _, p := range paths {
		if data, err := os.ReadFile(p); err == nil {
			content := strings.ToLower(string(data))
			if strings.Contains(content, "amazon") || strings.Contains(content, "ec2") ||
				strings.Contains(content, "nitro") || strings.Contains(content, "microsoft") ||
				strings.Contains(content, "azure") || strings.Contains(content, "google") ||
				strings.Contains(content, "gce") {
				return true
			}
		}
	}
	return false
}

// DetectCPUGeneration identifies whether the AMD EPYC CPU is Milan (Zen 3), Genoa (Zen 4), or Turin (Zen 5).
func DetectCPUGeneration() string {
	data, err := os.ReadFile("/proc/cpuinfo")
	if err != nil {
		return "milan"
	}
	content := strings.ToLower(string(data))
	switch {
	case strings.Contains(content, "genoa") || strings.Contains(content, "9004"):
		return "genoa"
	case strings.Contains(content, "turin") || strings.Contains(content, "9005"):
		return "turin"
	case strings.Contains(content, "milan") || strings.Contains(content, "7003"):
		return "milan"
	default:
		if strings.Contains(content, "cpu family\t: 26") {
			return "turin"
		}
		if strings.Contains(content, "cpu family\t: 25") {
			if strings.Contains(content, "model\t\t: 16") || strings.Contains(content, "model\t\t: 17") ||
				strings.Contains(content, "model\t\t: 18") || strings.Contains(content, "model\t\t: 19") {
				return "genoa"
			}
			return "milan"
		}
		return "milan"
	}
}

// GetAMDVCEKChain returns the VCEK cert chain for the given family.
func GetAMDVCEKChain(family string) string {
	fam := strings.ToLower(family)
	switch {
	case strings.Contains(fam, "milan") || strings.Contains(fam, "m6a") || strings.Contains(fam, "c6a"):
		return AMDChainMilanPEM
	case strings.Contains(fam, "genoa") || strings.Contains(fam, "m7a") || strings.Contains(fam, "c7a"):
		return AMDChainGenoaPEM
	case strings.Contains(fam, "turin"):
		return AMDChainTurinPEM
	default:
		return AMDChainMilanPEM
	}
}

// ResolveAMDVCEKChain returns the auto-detected VCEK cert chain.
func ResolveAMDVCEKChain() string {
	return GetAMDVCEKChain(DetectCPUGeneration())
}

// GetAMDVLEKChain returns the VLEK cert chain for the given family.
func GetAMDVLEKChain(family string) string {
	fam := strings.ToLower(family)
	switch {
	case strings.Contains(fam, "milan") || strings.Contains(fam, "m6a") || strings.Contains(fam, "c6a"):
		return AMDVLEKChainMilanPEM
	case strings.Contains(fam, "genoa") || strings.Contains(fam, "m7a") || strings.Contains(fam, "c7a"):
		return AMDVLEKChainGenoaPEM
	case strings.Contains(fam, "turin"):
		return AMDVLEKChainTurinPEM
	default:
		return AMDVLEKChainMilanPEM
	}
}

// ResolveAMDVLEKChain returns the auto-detected VLEK cert chain.
func ResolveAMDVLEKChain() string {
	return GetAMDVLEKChain(DetectCPUGeneration())
}

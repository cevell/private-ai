...
func BuildAttestationDocumentWithEvidence(binding BindingData, rawQuote []byte, certChain, platform, tlsCertPEM string, nonce string, gpuEv *GPUEvidence) *AttestationDocument {
	...
	var zeroKey [32]byte
	if binding.AuthPublicKey != zeroKey {
		doc.Predicate.AuthorizedPublicKey = hex.EncodeToString(binding.AuthPublicKey[:])
	}
	...
	// Compute composite user data binding all anchors: tlsFP, hpkePub, nonce, gpuDigest, authPub
	composite := ComputeCompositeUserData(binding.TLSKeyFingerprint, binding.HPKEPublicKey, nonceBytes, gpuEv, binding.AuthPublicKey)
	...
	// Bind tenant authorized public key as measured subject if present
	if binding.AuthPublicKey != zeroKey {
		authHash := sha256.Sum256(binding.AuthPublicKey[:])
		subjects = append(subjects, InTotoSubject{
			Name: "tenant-authorized-public-key",
			Digest: map[string]string{
				"sha256": hex.EncodeToString(authHash[:]),
			},
		})
	}

...
func TestDocument_AutomaticCompositeUserData(t *testing.T) {
	...
	// Predicate AuthorizedPublicKey should match authKey hex
	assert.Equal(t, hex.EncodeToString(authKey[:]), doc.Predicate.AuthorizedPublicKey)
	...
	assert.True(t, VerifyCompositeUserData(userDataArr, tlsFP, hpkeKey, nonce, gpuEv, authKey))
}

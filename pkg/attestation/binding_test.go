...
func TestComputeCompositeUserData_Structure(t *testing.T) {
	var tlsFP, hpkePub, nonce, authPub [32]byte
	for i := 0; i < 32; i++ {
		tlsFP[i] = byte(i + 1)
		hpkePub[i] = byte(i + 33)
		nonce[i] = byte(i + 65)
	...
	userData := ComputeCompositeUserData(tlsFP, hpkePub, nonce, gpuEv, authPub)
	...
	// Verify byte range 32..63 is SHA256(hpkePub || nonce || gpuDigest || authPub)
	gpuCombined := append([]byte(gpuEv.EvidenceReport), []byte(gpuEv.CertificateChain)...)
	expectedGPUDigest := sha256.Sum256(gpuCombined)

	expectedCombined := append(append(append(append([]byte{}, hpkePub[:]...), nonce[:]...), expectedGPUDigest[:]...), authPub[:]...)
...
func TestComputeCompositeUserData_NilAndEmptyGPU(t *testing.T) {
	...
	copy(authPub[:], "mock-auth-pub")

	// 1. When gpuEv == nil (CPU-only CVM)
	userDataNil := ComputeCompositeUserData(tlsFP, hpkePub, nonce, nil, authPub)
	assert.Equal(t, tlsFP[:], userDataNil[0:32])

	var zeroGPUDigest [32]byte
	combinedExpected := append(append(append(append([]byte{}, hpkePub[:]...), nonce[:]...), zeroGPUDigest[:]...), authPub[:]...)
	...
	userDataEmpty := ComputeCompositeUserData(tlsFP, hpkePub, nonce, &GPUEvidence{}, authPub)
...
func TestVerifyCompositeUserData_ValidationAndTamperResistance(t *testing.T) {
	...
	copy(authPub[:], "tenant-ed25519-auth-public-key--")
	...
	validUserData := ComputeCompositeUserData(tlsFP, hpkePub, nonce, gpuEv, authPub)

	// Valid verification
	require.True(t, VerifyCompositeUserData(validUserData, tlsFP, hpkePub, nonce, gpuEv, authPub), "Valid user data must verify successfully")

	// Tampered TLS fingerprint
	var tamperedTLS [32]byte
	copy(tamperedTLS[:], tlsFP[:])
	tamperedTLS[0] ^= 0xFF
	assert.False(t, VerifyCompositeUserData(validUserData, tamperedTLS, hpkePub, nonce, gpuEv, authPub), "Tampered TLS fingerprint must be rejected")

	// Tampered HPKE public key
	var tamperedHPKE [32]byte
	copy(tamperedHPKE[:], hpkePub[:])
	tamperedHPKE[0] ^= 0xFF
	assert.False(t, VerifyCompositeUserData(validUserData, tlsFP, tamperedHPKE, nonce, gpuEv, authPub), "Tampered HPKE key must be rejected")

	// Tampered nonce
	var tamperedNonce [32]byte
	copy(tamperedNonce[:], nonce[:])
	tamperedNonce[0] ^= 0xFF
	assert.False(t, VerifyCompositeUserData(validUserData, tlsFP, hpkePub, tamperedNonce, gpuEv, authPub), "Tampered nonce must be rejected")

	// Tampered tenant auth public key
	var tamperedAuthPub [32]byte
	copy(tamperedAuthPub[:], authPub[:])
	tamperedAuthPub[0] ^= 0xFF
	assert.False(t, VerifyCompositeUserData(validUserData, tlsFP, hpkePub, nonce, gpuEv, tamperedAuthPub), "Tampered tenant auth public key must be rejected")
	...
	assert.False(t, VerifyCompositeUserData(validUserData, tlsFP, hpkePub, nonce, tamperedGPUEv, authPub), "Spliced GPU report must be rejected")
	...
	assert.False(t, VerifyCompositeUserData(validUserData, tlsFP, hpkePub, nonce, tamperedGPUCerts, authPub), "Forged GPU certificate chain must be rejected")
}
...
func TestBindingData_MarshalComposite(t *testing.T) {
	...
	direct := ComputeCompositeUserData(tlsFP, hpkePub, nonce, gpuEv, authPub)

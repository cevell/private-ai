...
func FetchTDXReport(data [64]byte) ([]byte, string, string, error) {
	...
	// 3. Fallback to direct /dev/tdx_guest or /dev/tdx-guest device ioctl
	for _, devPath := range []string{"/dev/tdx_guest", "/dev/tdx-guest"} {
		if _, statErr := os.Stat(devPath); statErr == nil {
			if report, devErr := queryTDXDevice(devPath, data); devErr == nil && len(report) > 0 {
				return report, "", PlatformTDX, nil
			}
		}
	}

	// 4. Check if Intel TDX environment is present
...
func fetchTSMReport(expectedProvider string, data [64]byte) ([]byte, error) {
	...
	if _, err := os.Stat(tsmPath); err != nil {
		...
		_ = exec.Command("modprobe", "tsm_report").Run()
		_ = exec.Command("modprobe", "tdx-guest").Run()
	}
	if _, err := os.Stat(tsmPath); err != nil {
		return nil, err
	}

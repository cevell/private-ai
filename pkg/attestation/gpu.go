			...
			if data, err := os.ReadFile(vf); err == nil {
				...
				if v == "0x10de" {
					...
					if devData, err := os.ReadFile(filepath.Join(devDir, "device")); err == nil {
						...
						switch devID {
						case "0x2900", "0x2901", "0x2902", "0x2903":
							evidence.Model = "NVIDIA B200"
							evidence.Architecture = "Blackwell"
						case "0x2940", "0x2941":
							evidence.Model = "NVIDIA B100"
							evidence.Architecture = "Blackwell"
						case "0x2980", "0x2981":
							evidence.Model = "NVIDIA GB200"
							evidence.Architecture = "Blackwell"
	...
	if evidence.DeviceCount > 0 {
		if evidence.Architecture == "" {
			if strings.Contains(evidence.Model, "H100") || strings.Contains(evidence.Model, "H200") {
				evidence.Architecture = "Hopper"
			} else if strings.Contains(evidence.Model, "B100") || strings.Contains(evidence.Model, "B200") || strings.Contains(evidence.Model, "GB200") || strings.Contains(evidence.Model, "Blackwell") || strings.Contains(evidence.Model, "RTX PRO 6000") || strings.Contains(evidence.Model, "B40") {
				evidence.Architecture = "Blackwell"

...
let
  ...
  runtimeClosure = pkgs.closureInfo { rootPaths = [ pythonPkg uvPkg caCerts gccLibs pkgs.gcc pkgs.bash pkgs.coreutils pkgs.gnutar pkgs.gzip pkgs.glibc nftablesPkg ]; };
in
...
NVBLACKLIST
    ...
    # Install ldconfig compatibility script for Triton / CUDA library discovery
    cat << 'EOF' > "$root"/sbin/ldconfig
#!/bin/sh
if [ "$1" = "-p" ]; then
    echo "	libcuda.so.1 (libc6,x86-64) => /usr/lib/x86_64-linux-gnu/libcuda.so.1"
    echo "	libcuda.so (libc6,x86-64) => /usr/lib/x86_64-linux-gnu/libcuda.so"
    echo "	libnvidia-ml.so.1 (libc6,x86-64) => /usr/lib/x86_64-linux-gnu/libnvidia-ml.so.1"
    echo "	libnvidia-ptxjitcompiler.so.1 (libc6,x86-64) => /usr/lib/x86_64-linux-gnu/libnvidia-ptxjitcompiler.so.1"
    exit 0
fi
exit 0
EOF
    chmod 0755 "$root"/sbin/ldconfig
    ln -sf /sbin/ldconfig "$root"/usr/sbin/ldconfig
    ln -sf /sbin/ldconfig "$root"/bin/ldconfig
    ln -sf /sbin/ldconfig "$root"/usr/bin/ldconfig

    # Install C/C++ compiler toolchain (required for Triton and FlashInfer JIT compilation)
    ln -sf "${pkgs.gcc}/bin/gcc" "$root"/usr/bin/gcc
    ln -sf "${pkgs.gcc}/bin/g++" "$root"/usr/bin/g++
    ln -sf "${pkgs.gcc}/bin/cc" "$root"/usr/bin/cc
    ln -sf "${pkgs.gcc}/bin/c++" "$root"/usr/bin/c++
    ln -sf "${pkgs.gcc}/bin/gcc" "$root"/bin/gcc
    ln -sf "${pkgs.gcc}/bin/g++" "$root"/bin/g++
    ln -sf "${pkgs.gcc}/bin/cc" "$root"/bin/cc
    ln -sf "${pkgs.gcc}/bin/c++" "$root"/bin/c++

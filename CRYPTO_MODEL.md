# Cevell CVM: Cryptographic Specification & Verification Model
...
Cevell CVM enforces a strict zero-trust model where **neither the cloud host hypervisor nor the public API gateway is trusted with prompt confidentiality, model weights, or output tokens**.
...
│  - Verifies anti-splicing lock: REPORT_DATA commits to (TLS || HPKE || GPU || AuthPub)│
...
│  - Publicly trusted TLS endpoint (api.cevell.ai)                                  │
...
### 3.2 The Five Trust Anchors
A secure confidential inference node possesses five independent cryptographic identity anchors:
...
5. **`authPub`**: Enrolled Tenant Ed25519 Authorization Public Key (32 bytes).

Concatenating these raw anchors requires $32 \times 5 = 160\text{ bytes}$, exceeding the 64-byte silicon capacity.

### 3.3 The Canonical Mathematical Definition
To resolve this hardware constraint while achieving **transport channel binding, tenant identity locking, and CPU-GPU anti-splicing**, Cevell CVM implements a dual-partition composite construction:
...
$$\mathbf{REPORT\_DATA}[32:64] = \text{CompositeDigest} = \text{SHA-256}(\text{HPKEPublicKey} \parallel \text{SessionNonce} \parallel \text{GPUEvidenceDigest} \parallel \mathbf{AuthPublicKey})$$
...
|       SHA-256( hpkePub || nonce || gpuDigest || authPub )     |
+                       (Bytes 32..63)                          +
|                                                               |
+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
```
...
| `32..63` | 32 B | `CompositeDigest` | `SHA-256(hpkePub[32] \|\| nonce[32] \|\| gpuDigest[32] \|\| authPub[32])` | **Anti-Splicing & Tenant Identity Lock**. Concurrently binds the payload encryption key, client freshness nonce, NVIDIA GPU silicon evidence, and the enrolled tenant authorization public key. |

### 3.5 Preimage Concatenation Specification
The composite preimage for Bytes 32..63 is **strictly 128 bytes**, ordered as:
$$\text{Preimage} = \text{bytes}_{32}(\text{hpkePub}) + \text{bytes}_{32}(\text{nonce}) + \text{bytes}_{32}(\text{gpuDigest}) + \text{bytes}_{32}(\text{authPub})$$
...
4. `authPub` (32 bytes):
   - Raw binary bytes of the tenant's enrolled Ed25519 authorization public key.
   - Latched at launch from GCP instance metadata (`cevell-auth-pub`) or environment configuration. If unset: `[32]byte{0x00, 0x00, ... 0x00}`.
...
- **Hardware Ready-State Invariant**:
  ...
  - Cevell CVM automatically triggers this transition during early driver module initialization (`pkg/system/modules.go`) and validates adapter health before serving attestation requests.
...
Cevell CVM formats all attestation documents strictly according to the **in-toto Statement v1** standard.
...
```json
{
  ...
  "predicate": {
    ...
    "authorized_public_key": "<32_BYTE_ED25519_PUBKEY_HEX>",
...
#!/usr/bin/env python3
"""
Cevell CVM Independent Attestation & Anti-Splicing Verifier
...
def verify_attestation(attestation_path: str, expected_nonce_hex: str = None) -> bool:
    ...
    # Part B: Bytes 32..63 == SHA-256(hpkePub || nonce || gpuDigest || authPub)
    raw_auth_pub = pred.get("authorized_public_key", "00" * 32)
    auth_pub = bytes.fromhex(raw_auth_pub)
    assert len(auth_pub) == 32, f"Invalid authorized_public_key length: {len(auth_pub)} bytes"
    preimage = hpke_pub + nonce_bytes + gpu_digest + auth_pub
    assert len(preimage) == 128, f"Preimage length must be 128 bytes, got {len(preimage)}"
    expected_c32 = hashlib.sha256(preimage).digest()
    assert user_data[32:64] == expected_c32, "REPORT_DATA[32:64] does not match SHA-256(hpkePub || nonce || gpuDigest || authPub)!"
    print("[+] REPORT_DATA[32:64] matches SHA-256(hpkePub || nonce || gpuDigest || authPub).")
...
type StatementPredicate struct {
	...
	AuthorizedPublicKey string       `json:"authorized_public_key,omitempty"`
...
func VerifyAttestation(docBytes []byte, expectedNonceHex string) error {
	...
	authPubHex := stmt.Predicate.AuthorizedPublicKey
	if authPubHex == "" {
		authPubHex = hex.EncodeToString(make([]byte, 32))
	}
	authPub, err := hex.DecodeString(authPubHex)
	if err != nil || len(authPub) != 32 {
		return fmt.Errorf("invalid authorized_public_key hex or length != 32")
	}
	...
	// 2. Verify Bytes 32..63: Composite (HPKE || Nonce || GPU || AuthPub)
	preimage := append(append(append(append([]byte{}, hpkePub...), nonceBytes[:]...), gpuDigest[:]...), authPub...)
	expectedC32 := sha256.Sum256(preimage)
	if subtle.ConstantTimeCompare(userData[32:64], expectedC32[:]) != 1 {
		return fmt.Errorf("REPORT_DATA[32:64] does not match SHA-256(hpkePub || nonce || gpuDigest || authPub)")
	}
...
| **Key Authenticity** | `REPORT_DATA[32:64] == SHA-256(hpkePub \|\| nonce \|\| gpuDigest \|\| authPub)` | HPKE key or tenant authorized key substituted by untrusted Gateway. |
